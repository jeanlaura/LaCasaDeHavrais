<?php
    class dishes extends database {
        ...
        //Liste de catégories
        function listCat() {
            $sql = $this->database->prepare('SELECT * FROM `lcdh_categories`');
            $sql->execute();
            $result = $sql->fetchAll(PDO::FETCH_OBJ);
            return $result;
        }
        ...
        function displayDishes() {
            $sql = $this->database->prepare('
                SELECT `dishes_id`, `dishes_name`, `dishes_description`, `dishes_price`, `categories_name`, `subCategories_name`, `lcdh_dishes`.`categories_id`, `lcdh_dishes`.`subCategories_id`
                FROM `lcdh_dishes`
                INNER JOIN `lcdh_categories` ON `lcdh_categories`.`categories_id` = `lcdh_dishes`.`categories_id`
                INNER JOIN `lcdh_subcategories` ON `lcdh_subcategories`.`subcategories_id` = `lcdh_dishes`.`subcategories_id`
                WHERE `dishes_id` = :id
